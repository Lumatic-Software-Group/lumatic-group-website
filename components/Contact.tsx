          ...
          <div className={styles.messagingBlock}>
            ...
            <div className={styles.togglePill}>
              {MESSAGING_OPTIONS.map((opt) => (
                <button
                  key={opt.id}
                  className={`${styles.toggleBtn} ${activeMessaging === opt.id ? styles.toggleBtnActive : ""}`}
                  style={
                    activeMessaging === opt.id
                      ? ({ "--active-color": opt.color, "--active-shadow": opt.shadowColor } as React.CSSProperties)
                      : {}
                  }
                  onClick={() => setActiveMessaging(opt.id)}
                  aria-pressed={activeMessaging === opt.id}
                >
                  <span className={styles.toggleBtnIcon}>{opt.icon22}</span>
                  {opt.label}
                </button>
              ))}
            </div>
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className={styles.msgCta}
              style={
                {
                  "--cta-color": activeOption.color,
                  "--cta-shadow": activeOption.shadowColor,
